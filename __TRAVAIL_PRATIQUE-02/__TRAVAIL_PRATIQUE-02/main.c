...
#include "m_guichet.h"
#include "m_traiteur.h" //prends en compte m_file_chainee.h & m_liste_guichet.h dans l'appel
/*===============================================================================================*/
...
int main(void) {
	...
	t_block bloc = { 0 }; //bloc pour l'appel de get_bloc()
	t_block bloc_recept = { 0 }; //bloc pour la reception apres le traitement
	t_guichet guich = { 0 }; //<<======================================= À SUPPRIMER ULTERIEUREMENT
	t_traiteur trtr = { 0 }; //Module de traitement
	unsigned int id[] = { 0 }; //valeur de reception de l'identifiant
	int taille_rest = 0, bloc_in_tr = 0;

	init_decoupage();
	guich = init_guichet(TAILLE_FILE); //<<======================================= À SUPPRIMER ULTERIEUREMENT
	trtr = init_traiteur(TAILLE_FILE);
	id[0] = ajouter_fichier(NOM_FICHIER_0);
//	id[1] = ajouter_fichier(NOM_FICHIER_1);
	
	taille_restante_tot(id);

	do {
		bloc = get_bloc();

		entree_bloc(&trtr, bloc, TAILLE_FILE);

		bloc_recept = sortie_block(&trtr);

		if (bloc_recept.taille_bloc != 0) //lorsque le 
			print_bloc(&bloc_recept);

		taille_rest = get_taille_restante(id[0]);
		bloc_in_tr = get_nb_bloc_traiteur(&trtr);

	} while (taille_restante_tot(id) != 0 || get_nb_bloc_traiteur(&trtr) != 0); 
	/*condition d'arret:
	. Il n'a plus de fichier a decouper
	. il n'a plus de bloc dans le traiteur //analyser s'il est possible d'avoire des fichiers non totalement decouper lorsque que les guichets sont vides
	
/*
	do{//<<======================================= À SUPPRIMER ULTERIEUREMENT
	...
	} while (get_taille_restante(id[0]) || (guich.bloc_traite.taille_bloc != 0));
*/

	free_traiteur(&trtr);
	free_guichet(&guich);
	retirer_fichier(id[0]);
